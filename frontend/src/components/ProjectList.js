      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 rounded-lg shadow-sm border border-gray-100">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
              {user?.role === 'client' && mode === 'my' && (
                <Link
                    to="/post-project"
                    className="bg-brand-600 text-white px-4 py-2 rounded-lg hover:bg-brand-700 transition-colors text-sm font-medium flex items-center justify-center shadow-sm"
                >
                    + Post New Project
                </Link>
              )}
              <div className="relative">
                  ...
                  <input
                      ...
                      className="pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-brand-500 focus:border-brand-500 text-sm w-full sm:w-64"
                      value={searchTerm}
              ...
              <div className="relative">
                   ...
                   <select 
                        ...
                        className="pl-10 pr-8 py-2 border border-gray-300 rounded-lg focus:ring-brand-500 focus:border-brand-500 text-sm appearance-none bg-white"
                   >
          ...
          {filteredProjects.map((project) => (
            <div key={project.id} className="group bg-white rounded-xl shadow-sm hover:shadow-md border border-gray-100 transition-all duration-200 overflow-hidden">
              <div className="p-6">
                  ...
                  <div className="flex flex-wrap items-center gap-6 text-sm text-gray-500 border-t border-gray-50 pt-4">
                    ...
                    <Link 
                        ...
                        className="text-brand-600 font-medium hover:text-brand-700 text-sm flex items-center gap-1"
                    >
                        View Details →
                    </Link>
                  </div>
                  ...
                  {project.skills_required && project.skills_required.length > 0 && (
                      <div className="mt-4 flex flex-wrap gap-2">
                        {project.skills_required.map(skill => (
                            <span key={skill.id} className="bg-gray-50 text-gray-600 px-2 py-1 rounded-md text-xs font-medium border border-gray-200">
                                {skill.skill_name}
                            </span>
                        ))}
                      </div>
